...
public class TppBatchSplitter extends SftpBatchSplitter {
    ...
    public List<BatchSplit> splitBatch(Batch batch, Batch lastCompleteBatch, DataLayerI db,
        ...
        return copyToPermanentStorageAndCreateBatchSplits(orgDirs, batch, sourcePermDir, db, instanceConfiguration, dbConfiguration, batchDir);
    }
    ...
    private static List<BatchSplit> copyToPermanentStorageAndCreateBatchSplits(List<File> orgDirs, Batch batch, String sourcePermDir,
                                          DataLayerI db, DbInstanceEds instanceConfiguration, DbConfiguration dbConfiguration, String batchDir) throws Exception {
        ...
        for (File orgDir : orgDirs) {
            ...
            String manifestBulkReasonCodeFile = null;
            for (File splitFile : splitFiles) {
                if (splitFile.getName().equalsIgnoreCase(TppConstants.CODE_FILE) && TppConstants.FILTER_DUPLICATE_TPP) {
                    ...
                    manifestBulkReasonCodeFile = TppBulkDetector.detectBulkFromManifest(
                            batch, batchSplit, db, instanceConfiguration, dbConfiguration);
                    filterFilesForDuplicateData(storagePath, uniqueKey, orgId, splitFile, manifestBulkReasonCodeFile, batchDir);
                }
            }
            //copy everything to storage
            LOG.trace("Copying " + splitFiles.length + " files from " + orgDir + " to permanent storage");
            for (File splitFile : splitFiles) {
                if (splitFile.getName().equalsIgnoreCase(TppConstants.CODE_FILE) && manifestBulkReasonCodeFile != null) // File should have already been copied and skip
                    continue;
     ...
     */
    private static void filterFilesForDuplicateData(String storagePath, String uniqueKey,  String orgId,
                                                   File splitFile, String manifestBulkReason, String batchDir) throws Exception {
        LOG.info("Hashed File Filtering for SRCode using storagePath : " + storagePath + " uniqueKey " + uniqueKey  +  " orgId " +  orgId + " batchDir " + batchDir);
        try {
...
*/
            ...
            String tempTableName = ConnectionManager.generateTempTableName(FilenameUtils.getBaseName(storagePath));
